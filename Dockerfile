...
RUN set -ex \
    ...
    && mkdir -p conf-available conf-enabled mods-available mods-enabled sites-available sites-enabled \
    && mv extra/* conf-available/ \
    && rm -rf extra
...
RUN set -ex \
    && chown -Rf www-data:www-data "$HTTPD_PREFIX"
